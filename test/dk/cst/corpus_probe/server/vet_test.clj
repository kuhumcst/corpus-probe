(ns dk.cst.corpus-probe.server.vet-test
  "The startup self-checks: the CWB programs, the sort collation, and what
  the registry's corpora actually read as."
  (:require [babashka.fs :as fs]
            [clojure.test :refer [deftest is testing]]
            [dk.cst.corpus-probe.server.vet :as vet]
            [dk.cst.corpus-probe.test.cwb :refer [ctx phantom-ctx! when-cwb]]))

(deftest missing-tools!-test
  (is (empty? (vet/missing-tools! ["echo"])))
  (testing "a command this machine does not have is missing"
    (is (= ["no-such-command-xyzzy"]
           (vet/missing-tools! ["no-such-command-xyzzy"]))))
  (testing "a non-zero exit still counts as launchable, as every cwb-* tool
            gives one"
    (is (empty? (vet/missing-tools! ["false"])))))

(deftest tool-problems!-test
  (when-cwb
   (testing "an installation the suite already runs against is complete"
     (is (empty? (vet/tool-problems! ctx))))
   (testing "a cqp that is not there is named"
     (is (= ["no-such-cqp"]
            (vet/tool-problems! (assoc ctx :cqp "no-such-cqp")))))))

(deftest cache-problems!-test
  (testing "no cache configured is not a problem"
    (is (empty? (vet/cache-problems! {})))
    (is (empty? (vet/cache-problems! {:cache-dir ""}))))
  (testing "a directory that is not there yet is created"
    (let [dir (fs/file (fs/create-temp-dir) "cache")]
      (is (empty? (vet/cache-problems! {:cache-dir (str dir)})))
      (is (fs/directory? dir))))
  (testing "a usable cache is no problem"
    (is (empty? (vet/cache-problems! {:cache-dir (str (fs/create-temp-dir))}))))
  (testing "a directory that cannot be written fails every search, loudly"
    (let [dir (fs/create-temp-dir)]
      (.setWritable (fs/file dir) false)
      (is (= [:cache-unusable] (vet/cache-problems! {:cache-dir (str dir)})))
      (.setWritable (fs/file dir) true)))
  (testing "a budget larger than the disk is reported, not fatal"
    (let [petabyte (* 1024 1024 1024 1024 1024)
          dir      (str (fs/create-temp-dir))]
      (is (= [:cache-over-disk]
             (vet/cache-problems! {:cache-dir       dir
                                   :cache-max-bytes petabyte})))
      (testing "and an unusable directory outranks it"
        (let [f (fs/file (fs/create-temp-dir) "cache")]
          (spit f "x")
          (is (= [:cache-unusable]
                 (vet/cache-problems! {:cache-dir       (str f)
                                       :cache-max-bytes petabyte})))))))
  (testing "a path that is a file rather than a directory is reported, for
            start! to run without it"
    (let [f (fs/file (fs/create-temp-dir) "cache")]
      (spit f "x")
      (is (= [:cache-unusable] (vet/cache-problems! {:cache-dir (str f)}))))))

(deftest collation-test
  (when-cwb
   (testing "under a Danish locale CQP's pipeline and our collator agree"
     (is (= (vet/pipeline-order! "da_DK.UTF-8")
            (vet/collator-order "da_DK.UTF-8")))
     (is (empty? (vet/collation-problems! {:sort-locale "da_DK.UTF-8"}))))
   (testing "and they order æ, ø and å after z, not in byte order"
     (is (= ["4" "2" "1" "5" "3"] (vet/pipeline-order! "da_DK.UTF-8"))))
   (testing "a locale this machine does not have collates by bytes instead"
     (is (= ["4" "2" "3" "1" "5"] (vet/pipeline-order! "zz_ZZ.UTF-8")))
     (is (= [:collation-mismatch]
            (vet/collation-problems! {:sort-locale "zz_ZZ.UTF-8"}))))
   (testing "so does asking for byte order outright"
     (is (= [:collation-mismatch] (vet/collation-problems! {:sort-locale "C"})))))
  (testing "no configured locale is a problem of its own, not a pass"
    (is (= [:sort-locale-unset] (vet/collation-problems! {})))
    (is (= [:sort-locale-unset] (vet/collation-problems! {:sort-locale ""})))))

(deftest collation-problems!-test
  (testing "a pipeline that cannot run at all is told apart from a mismatch"
    (with-redefs [vet/pipeline-order! (constantly nil)]
      (is (= [:pipeline-broken]
             (vet/collation-problems! {:sort-locale "da_DK.UTF-8"}))))))

(deftest corpus-problem!-test
  (when-cwb
   (testing "a corpus CWB can read vets clean"
     (is (nil? (vet/corpus-problem! ctx {:id "probe"}))))
   (testing "an entry CWB has no data for is named, uppercase, as undefined"
     (is (= ["PROBE" :undefined]
            (vet/corpus-problem! (phantom-ctx!) {:id "probe"}))))
   (testing "another failure carries its type, which names no server path"
     (let [[id reason] (vet/corpus-problem! (assoc (phantom-ctx!)
                                                   :cqp "no-such-cqp")
                                            {:id "probe"})]
       (is (= "PROBE" id))
       (is (contains? #{:unreadable :timeout :cqp :misaligned} reason))
       (is (not= :undefined reason))))))

(deftest registry-problems!-test
  (when-cwb
   (testing "a registry of nothing but a phantom reports it"
     (is (= [["PROBE" :undefined]] (vet/registry-problems! (phantom-ctx!)))))
   (testing "the dev corpora read clean"
     ;; naming them, since the gitignored dev registry may hold more
     (let [broken (set (map first (vet/registry-problems! ctx)))]
       (is (not-any? broken ["PROBE" "VISER" "TALER"])))))
  (testing "a registry path that names nothing reads as empty, not as fine"
    ;; a mistyped :registry looks exactly like an empty one, so the caller
    ;; gets no corpora rather than an error
    (is (empty? (vet/registry-problems! {:registry "/no/such/registry"})))))
